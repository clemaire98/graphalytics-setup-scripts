...
PLUGIN_DIR=graphalytics-platforms-powergraph
...
find $SCRIPT_DIR/ -name "graphalytics-*-*-*-bin.tar.gz" | xargs tar -C $SCRIPT_DIR/ -xzf

echo Removing remaining tar.gz.
find $SCRIPT_DIR/ -name "graphalytics-*-*-*-bin.tar.gz" | xargs rm

COMPILED_DIR=$(find $SCRIPT_DIR/ -maxdepth 1 -name "graphalytics-*-*-*" | xargs readlink -f)
